    ...
    firstCellHeight_(readScalar(dict.lookup("firstCellHeight")))
{
	...
	forAll(faceCenterPoints, pointJ)
	{
        ...
        if(mag(currentPt.z()-faceCenterPoints[pointJ].z()) > .1 )
 	   {
		    ...
		    {
         	    FatalErrorIn("lapseRateProfileTemperatureInletFvPatchScalarField")
...
lapseRateProfileTemperatureInletFvPatchScalarField::lapseRateProfileTemperatureInletFvPatchScalarField
(
    const lapseRateProfileTemperatureInletFvPatchScalarField& fcvpvf,
    const DimensionedField<vector, volMesh>& iF
)
:
...
makePatchTypeField(fvPatchVectorField, lapseRateProfileTemperatureInletFvPatchScalarField);
