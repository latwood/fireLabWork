...
imgDir = mainDir+"/Pics/energyEqn/vshapedvalley-flatbot-notilt/buoyantBoussinesqPimpleFoam/Coarse/1mph0deg-reformedConditions/glyphsFull-TopViewElevNeg45-Rescaling/"
...
UlegendPosition = [0.475,0.12]#[0.3,0.12]	# this will need to be adjusted a bunch
TlegendPosition = [0.475,0.12]#[0.3,0.12]	# this will need to be adjusted a bunch
...
viewCameraZ = -900	# where in the z direction from the center do you want to position the object?
cameraElev = -45	# the tilt to give the view. It seems that if 45 it is easy since z and y are now 45 degrees each way, so to go up you need to go the same distance in both of those. Going to be tricky to get the right changes of x,y,z stuff based on elevation or camera position. There has got to be some kind of vector math that makes it easier to figure out though.
