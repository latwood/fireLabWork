...
imgDir = mainDir+"/Pics/energyEqn/vshapedvalley-flatbot-notilt/buoyantBoussinesqPimpleFoam/Coarse/1mph0deg-reformedConditions/TglyphSlice-SouthView-OffsetNeg1000-Rescaling/"
...
ySliceOffset = -1000			# this is the offset of the y slice starting point
...
glyphYsliceStride = 2		# this is the every Nth number of points to use for showing vectors
...
dpYslice.SetScalarBarVisibility(view, True)
scalarbarYslice.Position = TlegendPosition	# have to reset this every time you make the scalar bar visible again, SetScalarBarVisibility resets the legend position
dpGlyphYslice.SetScalarBarVisibility(view,True)
scalarbarGlyphYslice.Position = UlegendPosition
Render()
SetActiveSource(glyphYslice)
...
for i in range(0,len(timeSteps)):
	...
	Render()
	ColorBy(dpYslice, ('CELLS','T'))
	dpYslice.RescaleTransferFunctionToDataRange()	#looks like if you throw a True into this final parenthesis, it only rescales if the value is greater
	dpYslice.SetScalarBarVisibility(view, True)
	scalarbarYslice.Position = TlegendPosition	# have to reset this every time you make the scalar bar visible again, SetScalarBarVisibility resets the legend position
	ColorBy(dpGlyphYslice, ('POINTS','GlyphVector'))
	dpGlyphYslice.RescaleTransferFunctionToDataRange()
