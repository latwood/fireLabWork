...
tiffFileName <- paste(mainDir,"mesoscaleWeatherPaper1.tif",sep="")
demPlotFileName <- paste(mainDir,"mesoscaleWeatherPaper1-demPlot.png",sep="")
sidePlotFileName <- paste(mainDir,"mesoscaleWeatherPaper1-sideDemPlot.png",sep="")
...
hy <- function(y) {
  if (abs(y) <= L_y) {
    h_y <- 1
  } else if (L_y < abs(y) && abs(y) < (L_y+S_y)) {
    h_y <- 0.5+0.5*cos((pi*(abs(y)-L_y))/S_y)
  } else if(abs(y) >= (L_y+S_y)) {
    h_y <- 0
  }
  return (h_y)
}
